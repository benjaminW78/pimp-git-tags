...
(async function () {
    ...
    if (lastTag) {
        let t = await inquirer.prompt([{
            type: 'list',
            name: 'bumpType',
            message: 'Select the type of bump you want to proceed',
            default: 'minor',
            pageSize: 10,
            choices: [new inquirer.Separator(), 'major', new inquirer.Separator(), 'minor', new inquirer.Separator(), 'patch', new inquirer.Separator(), 'exit program'],
            prefix: 'Bump tag version \n'
        }])
        ...
        let t = await inquirer.prompt([{
            type: 'input',
            name: 'value',
            message: 'Type the number of your first annotated Tag : ',
            default: '0.1.0'
        }])
...
async function whereToCreateNewTag(newTag) {
    ...
    let choice = await inquirer.prompt([{
        type: 'confirm',
        name: 'value',
        message: 'Do you want to add ' + newTag + ' to the current git project and at current HEAD position',
        prefix: 'Generate new git tag \n'
    }])
    if (choice.value) {
        ...
        let choice1 = await inquirer.prompt([{
            type: 'confirm',
            name: 'value',
            message: 'Do you want to add ' + newTag + ' to a specific commit number',
            prefix: 'Generate new git tag custom\n'
        }])
        if (choice1.value) {
            let commit = await inquirer.prompt([{
                type: 'input',
                name: 'value',
                message: 'Please provide a commit hash currently existing on your computer ==> : ',
                prefix: 'Generate new git tag custom\n'
            }])
